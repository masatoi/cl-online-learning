...
(declaim (ftype (function (dvec) (double-float 0.0d0)) distance-to-origin))
(defun distance-to-origin (x)
  (declare (type dvec x)
	   #+allegro (:faslmode :immediate))
  (let ((result 0.0d0))
    (declare (type (double-float 0.0d0) result))
    (do-vec (ex x :type double-float)
      (incf result (* ex ex)))
    (sqrt result)))
...
(declaim (ftype (function (dvec dvec) (double-float 0.0d0)) euclid-distance))
(defun euclid-distance (x y)
  (declare (type dvec x y)
	   #+allegro (:faslmode :immediate))
  (assert (= (length x) (length y)))
...
(declaim (ftype (function (dvec dvec) (double-float 0.0d0)) manhattan-distance))
...
(declaim (ftype (function (dvec dvec) (double-float 0.0d0)) cosine-distance))
