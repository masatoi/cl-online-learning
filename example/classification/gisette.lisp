...
(defparameter arow-learner (make-arow gisette-dim 10.0))
(loop repeat 20 do
  (train arow-learner gisette-train)
  (test arow-learner gisette-test))

(defparameter scw-learner (make-scw gisette-dim 0.9 0.1))
(loop repeat 20 do
  (train scw-learner gisette-train)
  (test scw-learner gisette-test))

(defparameter sgd-learner (make-lr+sgd gisette-dim 0.00001 0.01))
(loop repeat 20 do
  (train sgd-learner gisette-train)
  (test sgd-learner gisette-test))

; α = 0.001, β1 = 0.9, β2 = 0.999 and ε = 10^-8
(defparameter adam-learner (make-lr+adam gisette-dim 0.000001 0.001 1.e-8 0.9 0.99))
(loop repeat 20 do
